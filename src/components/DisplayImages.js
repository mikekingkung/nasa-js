                ...
                images.map(data => (
                  <div>
                    ...
                    <Grid container xs={12} spacing={6} className="classes.container">
                      ...
                      <Typography component="div">
                        <Box style={{ border: "1px solid blue", margin: "10px"}} textAlign="justify" m={10} color="#ffffff"  fontFamily="Orbitron" fontStyle="sans-serif">
                       {data.image ? <p  style={{ padding: 20 }} >{data.image.description} key={data.image.id}</p> :<div/> }
                       {data.video ? <p style={{ padding: 20 }} >{data.video.description} key={data.video.id}</p> : <div/> }
                       </Box> 
                       </Typography>                     
                    </Grid>
                    <Grid item xs={6}  justify="center" alignItems ="center" className="classes.missioncontextcontainer">
                      {data.image ? <img src={data.image.url} alt="image" width="500" height="500" key={data.image.id}></img> : <p/> }
